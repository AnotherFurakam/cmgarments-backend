...
export class SaleService {
  ...
  async findAllDetails(
  ...
  ): Promise<PaginationResponseDto<GetSaleDetailDto[]>> {
    ...
    if (page > pages) {
      if (total === 0)
        throw new NotFoundException('Aún no hay detalles de venta');
    ...
    const saleDetailData = saleDetails.map((sd: SaleDetail) =>
      plainToInstance(GetSaleDetailDto, sd),
    );
